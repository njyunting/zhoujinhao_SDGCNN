% 文件夹名称变量
folder_name1 = 'para_class'; % 文件夹名

% 读取点云数据
point_cloud = load(sprintf('D:/yunting/data/bigpicture2/%s.txt', folder_name1));

% 提取点云坐标和类别标签
point_cloud_X = point_cloud(:, 1);
point_cloud_Y = point_cloud(:, 2);
point_cloud_Z = point_cloud(:, 3);
labels = point_cloud(:, 8); % 第七列为类别标签
...
% 定义类别标签和对应颜色
label_colors = [
    0, 0.8, 0.8, 0.8;  % 标签0，粉色
    1, 0.5, 0.5, 0.5;    % 标签1，灰色
    2, 1, 1, 0.5;      % 标签2，黄色
    3, 1, 0.4, 0.74;  % 标签3，粉色
    5, 0.6, 0.3, 0;    % 标签5，棕色
    6, 1, 0, 0;        % 标签6，红色
...
% 设置不同类别的稀疏度
sparsity_map = containers.Map([0, 1, 2, 3, 5, 6, 8, 9], [0.1, 0.015, 0.07, 0.06, 0.1, 0.1, 0.04, 0.04]);
...
for k = 1:length(Node1)
    ...
    plot3(x, y, z, 'Color', color, 'LineWidth', 0.3);
end
